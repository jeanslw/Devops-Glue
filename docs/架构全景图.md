...
### 水平扩展 Web 层（docker-compose + K8s 参考）

compose 文件用 YAML 锚点共享 web 配置，副本无需复制多份：

- **`x-web-common: &web-common`** 收纳 web 副本的全部公共项（镜像、`env_file`、`TZ`、所有卷挂载、重启策略、健康检查、`depends_on: mysql`）。新增副本 = 一个 service 段 + 一行 `<<: *web-common` 合并——注释的 `devops-glue-web-2` 即现成模板。
- **`front-nginx`**（默认注释）是可选的前端负载均衡——`config/docker/nginx-lb.conf` 定义对 `devops-glue:80` 的轮询 `upstream`（每加一个副本加一行 `server`），透传 `X-Forwarded-*` / `Upgrade` 头，并 `proxy_buffering off`。单实例部署无需它：`devops-glue` 直接映射 `8080:80`。
- **无需粘滞会话**：鉴权走 token（无 `$_SESSION`），任意副本可服务任意请求。LB 之后需设 `TRUSTED_PROXY_HOPS=1`——否则应用读到的是 LB 的 IP 而非客户端（登录锁桶 / 审计日志）。

**Kubernetes 参考清单**（`config/k8s/devops-glue.yaml`）复刻此拓扑——web `Deployment`（`replicas: 3`）、单 worker、`front-nginx` `LoadBalancer` 入口、`app.env` `Secret`，另有可选集群内 MySQL（`StatefulSet`）与 RWX 备份/SQLite PVC。K8s 下 OIDC 签名私钥必须注入固定 `OIDC_RSA_PRIVATE_KEY`（多行 Secret），否则各副本各自生成不同私钥导致 SSO 失效。
