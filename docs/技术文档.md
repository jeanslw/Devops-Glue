...
#### 5.9.4 RBAC 权限系统（v2.4.1）

系统采用基于角色的访问控制（RBAC），使用三张数据库表：

| 表名 | 用途 |
|---|---|
| `roles` | 角色定义：`id`, `name`, `description`, `is_system`, `created_at` |
| `permissions` | 权限目录：`id`, `key`（点分隔，如 `cd.deploy.k8s`）、`parent_key`, `description`, `created_at` |
| `role_permissions` | 多对多关联：`role_id`, `permission_key` |
...
| CD 一级 | 8 | `cd.build-manage`, `cd.deploy-manage`, `cd.server-manage`, `cd.webshell`, `cd.deploy-record`, `cd.image-registry`, `cd.resource-monitor`, `cd.notification-manage` |
| CD 二级 | 7 | `cd.deploy.single`/`docker`/`k8s`, `cd.monitor.app`/`system`/`custom`/`alert` |

**权限隐含关系**（`AppConfig::IMPLIED_PERMISSIONS` + 前端 `IMPLIED_PERMISSIONS`）：
- `cd.build-manage` → `ci.trigger`（构建管理隐含触发构建）

**数据库种子数据：**
- 内置角色和权限通过 `ensureTables()` 中 `REPLACE INTO` 写入
- `AppConfig.php` 的 `DEFAULT_PERMISSIONS` 存储英文规范名称（UI 通过 i18n 键渲染显示）
- 新增权限：`permissions` 表插入一行 + 中英 lang 文件添加翻译键

**API 端点**（v2.4.1）：
- `GET /api/admin/me/permissions` — 返回当前用户角色和权限列表（仅需有效 token，不需 admin 权限）
- `super_admin` 返回 `permissions: "*"`（通配符）；其他角色返回权限键数组

**权限检查逻辑**（`hasPermission($permKey)`）：
```php
if ($role === 'super_admin') return true;  // 硬编码旁路
// 否则：JOIN roles → role_permissions → permissions 查询
```
