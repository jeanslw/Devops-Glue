...
| `pipeline_iid` | INTEGER NOT NULL (PK) | Pipeline 内部编号（整数，对齐 `ci_pipeline_tags` 约束） |
| `status` | TEXT | 构建状态 |
| `sha` | TEXT | Commit SHA |
...
- 镜像 tag 不存此表，而是复用 `ci_pipeline_tags` 表（由 CD 层共享）
...
- **`ci_pipeline_tags` 表**：复用现有表存储镜像 tag，CD 层通过 `GET /api/build/{path}/tag` 读取。
...
| `tag` | string | ✅（success） | 镜像 tag，`status=success` 时必填并写入 `ci_pipeline_tags` |
...
> `status=success` 时 `tag` 与可解析的 `harbor_repository` 必填，写入 `ci_pipeline_tags`（project、pipeline_iid、tag、harbor_repository、finished_at、status）；重复上报按覆盖（UPDATE）处理。
