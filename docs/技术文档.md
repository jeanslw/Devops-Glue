...
// 否则：loadUserPermissions() 从 role_permissions JOIN 拉取 perm_key，
//       expandPermissions() 展开隐含，最后 in_array 对比
```
用户的权限数组展开后会存于 `$this->userPermissions`，一次请求内复用。
