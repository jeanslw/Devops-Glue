...
- key=`build_mode`，value ∈ {jenkins, gitlab_ci, both}（控制 pull-based CI）
- key=`custom_push_enabled`，value ∈ {0,1}（独立布尔开关，控制 push-based CI）

> **正交设计：** `build_mode`（jenkins/gitlab_ci/both）仅控制 pull-based CI 通道；`custom_push_enabled` 是独立的布尔开关，两者互不依赖，可同时启用。`build_mode` 下拉框三选项始终可选，`custom_push_enabled` 独立勾选。详见 [§5.10](#510-custom_push-自定义推送-ci-模式)。
...
| 控制 | jenkins / gitlab_ci / both | 独立布尔开关 |
| 关系 | 互相独立，可同时启用 | 互相独立，可同时启用 |

例如：`build_mode=jenkins` + `custom_push_enabled=true` 表示 Jenkins 拉取式 CI 和 Custom_Push 推送式 CI 同时生效。
...
- **下拉菜单**：`build_mode` 三选项（jenkins/gitlab_ci/both）始终可选
