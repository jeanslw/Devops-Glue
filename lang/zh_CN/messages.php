<?php
/**
 * 中文语言包（默认）
 * 组织方式：按 domain.key 扁平化，便于使用 Translator
 */
return [
    // ── 通用 ──
    'common.ok'       => '正常',
    'common.error'    => '错误',
    'common.yes'      => '是',
    'common.no'       => '否',
    'common.save'     => '保存',
    'common.cancel'   => '取消',
    'common.confirm'  => '确认',
    'common.delete'   => '删除',
    'common.edit'     => '编辑',
    'common.add'      => '新增',
    'common.search'   => '搜索',
    'common.loading'  => '加载中…',
    'common.success'  => '操作成功',
    'common.failed'   => '操作失败',
    'common.enabled'  => '启用',
    'common.disabled' => '禁用',
    'common.pending'  => '待启用',
    'common.unknown'  => '未知',
    'common.na'          => '不适用',
    'common.unreachable' => '不可达',
    'common.not_configured' => '未配置',
    'common.network_error' => '网络错误',
    'common.detecting' => '检测中…',

    // ── HTTP 错误页面 ──
    'error.400' => '请求参数错误',
    'error.401' => '未授权',
    'error.403' => '禁止访问',
    'error.404' => '页面不存在',
    'error.405' => '请求方法不允许',
    'error.408' => '请求超时',
    'error.429' => '请求过于频繁',
    'error.500' => '服务器内部错误',
    'error.502' => '网关错误',
    'error.503' => '服务不可用',
    'error.504' => '网关超时',
    'error.back_home' => '返回首页',
    'error.go_back'   => '返回上一页',
    'error.refresh'   => '刷新页面',

    // ── 认证 ──
    'auth.login'              => '登录',
    'auth.logout'             => '退出',
    'auth.not_logged_in'      => '未登录',
    'auth.token_invalid'      => 'token 无效',
    'auth.token_expired'      => 'token 已过期，请重新登录',
    'auth.token_store_failed' => '登录状态保存失败，请检查数据库连接',
    'auth.wrong_credentials'  => '账号或密码错误',
    'auth.login_locked'       => '登录失败次数过多，请 15 分钟后再试',
    'auth.password_updated'   => '密码已更新，请重新登录',
    'auth.old_password_wrong' => '旧密码错误',
    'auth.new_password_short' => '新密码至少 8 位',
    'auth.please_enter_credentials' => '请输入账号和密码',
    'auth.no_cd_access'       => '该账号无 CD 系统权限',
    'auth.no_ci_access'       => '该账号无 CI 系统权限',
    'auth.no_system_access'   => '该账号无此系统权限',
    'auth.please_login_first' => '请先登录 API 文档',
    'auth.change_password'    => '修改密码',
    'auth.old_password'       => '旧密码',
    'auth.new_password'       => '新密码',
    'auth.confirm_password'   => '确认新密码',
    'auth.update_password'    => '更新密码',
    // LDAP
    'auth.ldap_not_bound'     => '该 LDAP 账号未绑定本系统用户，请联系管理员绑定后再登录',
    'auth.user_disabled'      => '该账号已被停用，请联系管理员',

    // ── 映射管理 ──
    'map.job_name_required'      => 'job_name 为必填字段',
    'map.job_name_required_param' => 'job_name 为必填参数',
    'map.original_name_required' => '_original_job_name 为必填字段',
    'map.already_exists'         => '映射已存在，请使用编辑功能',
    'map.not_found'              => '映射不存在',
    'map.no_data'                => '暂无映射配置，点击上方按钮新增',
    'map.no_data_topo'           => '暂无映射数据',
    'map.no_data_topo_hint'      => '请先通过「表格」视图新增或自动发现映射关系',
    'map.new'                    => '新增映射',
    'map.edit'                   => '编辑映射',
    'map.config'                 => '映射配置',
    'map.total'                  => '总映射数',
    'map.active'                 => '活跃项目',
    'map.git_platforms'          => 'Git 平台',
    'map.image_repos'            => '镜像仓库',
    'map.table_view'             => '表格',
    'map.topology_view'          => '拓扑图',
    'map.search_placeholder'     => '搜索 Job/仓库/Remote…',
    'map.all_build_sources'      => '全部构建源',
    'map.all_platforms'          => '全部平台',
    'map.all_types'              => '全部类型',
    'map.all_status'             => '全部状态',
    'map.job_name'               => 'Job 名称',
    'map.build_source'           => '构建源',
    'map.git_platform'           => 'Git 平台',
    'map.git_remote'             => 'Git Remote',
    'map.harbor_repo'            => 'Harbor 仓库',
    'map.status'                 => '状态',
    'map.actions'                => '操作',
    'map.auto_discover'          => '自动发现',
    'map.discover_confirm'       => '将自动扫描 Jenkins/GitLab CI 项目并导入映射表，已有项目不会重复。确定？',
    'map.discover_scanning'      => '正在扫描，请稍候…',
    'map.discover_result'        => '发现 {found} 项，新增 {saved} 项',
    'map.discover_failed'        => '扫描失败',
    'map.discover_disabled'      => '自动发现功能未启用（Jenkins 不可用）',
    'map.domain'                 => 'domain',
    'map.repo'                   => '代码仓库',
    'map.ci'                     => 'CI 系统',

    // ── Git ──
    'git.query_failed' => 'Git 查询失败',

    // ── 构建 ──
    'build.scan_failed'     => '扫描失败',
    'build.query_failed'    => '查询失败',
    'build.modify_failed'   => '修改失败',
    'build.save_failed'     => '保存失败',
    'build.load_failed'     => '加载失败',
    'build.provider_not_configured' => "Build 系统 '{provider}' 未配置",
    'build.provider_not_configured_registered' => "Build 系统 '{provider}' 未配置（已注册: {registered}）",
    'build.mode_required'   => 'mode 必须为 jenkins / gitlab_ci / gitea_ci 之一',
    'build.jenkins_unavail' => 'Jenkins 不可用（未配置 url），请先检查配置文件',
    'build.gitlab_ci_unavail' => 'GitLab CI 不可用（未配置 base_url / token），请先检查配置文件',
    'build.gitea_ci_unavail' => 'Gitea Actions 不可用（未配置 base_url / token），请先检查配置文件',
    'build.mode_label'      => '构建模式',
    'build.mode_note_1'     => '可多选，切换立即生效，将影响后续构建任务。',
    'build.mode_note_2'     => '取消勾选某个 CI 会将其映射记录停用（降为「待启动」）；未配置的 CI 不会显示。',
    'build.custom_push_note_1' => '用户 CI 推送构建结果，Devops-Glue 只接收回写。',
    'build.custom_push_note_2' => '关闭后，已激活的 Custom_Push 映射将降为待定状态。',
    'build.stale_cleanup_note_on'  => '启用：将以 Harbor 为准，删除系统中已不存在的 tag 记录，Harbor 库的 tag 不会删除。',
    'build.stale_cleanup_note_off' => '关闭：停止清理，系统中可能有 tag 实际不存在的情况。',
    'build.mode_jenkins'    => '⚡ Jenkins',
    'build.mode_gitlab_ci'  => '🐺 GitLab CI',
    'build.mode_gitea_ci'   => '🦎 Gitea Actions',
    'build.mode_custom_push' => '📤 Custom_Push',
    'build.custom_push_toggle' => '📤 启用 Custom_Push',
    'build.custom_push_no_provider' => '尚未在 settings.php 中配置 custom_providers，无法启用 Custom_Push',
    'build.mode_both'       => 'Jenkins + GitLab',
    'build.mode_hint'       => '可多选，修改后即时生效。取消勾选某个 CI 会将不再匹配的映射记录停用（降为「待定」），未配置的 CI 不会显示。',
    'build.mode_saved'      => '已保存',
    'build.stale_tag_cleanup_toggle' => '🧹 启用清理过期 tag 记录',
    'build.stale_tag_cleanup_title'  => '过期 tag 清理',
    'build.tag_log_keyword_title' => '🔍 镜像 Tag 日志关键字',
    'build.tag_log_keyword_note'  => '拉取式记录「镜像 Tag」列未命中扫描回写时，用该关键字在构建日志中匹配推送成功片段（大小写不敏感，`|` 分隔多个，留空恢复默认 digest）。',
    'build.backfill_tag_toggle'   => '🔄 启用镜像 Tag 日志回填',
    'build.backfill_note_on'      => '启用：回填 cron 将把日志推导的 tag（经 Harbor 校验存在）补写进产物 tag 记录，仅补缺失项，不覆盖扫描结果。',
    'build.backfill_note_off'     => '关闭：仅懒加载展示，不回写产物 tag 记录。',
    'build.resolve_tag_missing_id' => '缺少 pipeline id 参数',

    // ── push 记录（Custom_Push 构建记录） ──
    'push.job_name'        => 'job_name',
    'push.pipeline_iid'    => 'pipeline_iid',
    'push.status'          => 'status',
    'push.sha'             => 'sha',
    'push.variables_json'  => 'variables_json',
    'push.log_url'         => 'log_url',
    'push.web_url'         => 'web_url',
    'push.finished_at'     => 'finished_at',
    'push.tag'             => 'tag',
    'push.no_records'      => '暂无 push 记录',
    'push.view_log'        => '查看日志',
    'push.load_failed'     => '加载 push 记录失败',
    'pull.project_label'   => '项目',
    'pull.confirm_btn'     => '确定',
    'pull.refresh'         => '刷新',
    'pull.select_first'    => '请先选择项目',
    'pull.no_projects'     => '暂无可查看的拉取式项目',
    'pull.no_records'      => '该项目暂无构建记录',
    'pull.load_failed'     => '加载构建记录失败',
    'pull.pipeline'        => '流水线',
    'pull.branch'          => '分支',
    'pull.commit'          => 'Commit',
    'pull.tag'             => '镜像Tag',
    'pull.time'            => '时间',
    'pull.view'            => '查看',
    'pull.log'             => '日志',
    'pull.log_title'       => '构建日志',
    'pull.no_log'          => '日志不可用',
    'pull.log_load_failed' => '日志加载失败',
    'pull.resolve_tag' => '解析',
    'pull.resolve_tag_failed' => '解析 Tag 失败',
    'pull.reparse_tag' => '重新解析',
    'build.commit_status_missing_fields' => 'sha / state / context / description 均为必填',
    'build.commit_status_invalid_state'  => "state 必须为: {states}",
    'build.commit_status_invalid_sha'    => 'sha 必须为 7~64 位十六进制（合法 commit SHA）',
    'build.commit_status_no_git_platform' => "项目 '{path}' 未配置 git_platform，无法回写 commit status",
    'build.git_platform_not_available' => "Git 平台 '{platform}' 未配置或不可用",
    'build.no_harbor_repo'       => "项目 '{path}' 未配置 harbor_repository",
    'build.harbor_repo_format_error' => "harbor_repository 格式错误: {repo}",
    'build.no_tag_available'     => '缺少 tag 参数且 Harbor 无可用 tag',
    'build.tag_not_found'        => "Tag '{tag}' 在 Harbor 仓库 '{repo}' 中不存在或已被删除",
    'build.scan_sync_identity'   => 'pipeline_iid 与 sha 必须成对提供，不能只传其一',

    // ── Harbor ──
    'harbor.trigger_failed' => '触发扫描失败',
    'harbor.report_failed'  => '获取扫描报告失败',
    'harbor.scan_in_progress'        => '扫描已在运行中，请稍后查看报告',
    'harbor.scan_not_enabled'        => '镜像扫描功能未启用，请联系管理员',
    'harbor.scan_report_not_enabled' => '镜像扫描功能未启用，无法获取报告',

    // ── 平台版本 ──
    'platform.versions_empty' => 'versions 不能为空',
    'platform.config' => '平台对接版本配置',
    'platform.hint'   => '以下版本号用于构建各平台的 API 请求路径（如 /api/v4）。修改后立即生效，留空则恢复默认值。GitHub 版本通过 HTTP Header 传递，仅影响显示。',
    'platform.platform' => '平台',
    'platform.default'  => '默认值',
    'platform.source'   => '来源',
    'platform.current'  => '当前版本',
    'platform.note'     => '说明',
    'platform.gitlab_desc'  => 'API v4，2017年至今无破坏性变更',
    'platform.gitee_desc'   => 'API v5，SaaS 平台端点稳定',
    'platform.github_desc'  => '通过 X-GitHub-Api-Version Header 传递，仅影响显示',
    'platform.gitea_desc'   => 'API v1，自建平台',
    'platform.harbor_desc'  => '支持 v1.10 / v2.x 自动探测；机器人账户需 v2.2.0+',

    // ── 安全审计 ──
    'security.scan_results' => '安全审计结果',
    'security.search_placeholder' => '搜索项目…',
    'security.no_records'  => '暂无审计记录',
    'security.passed'      => '通过',
    'security.failed'      => '失败',
    'security.error'       => '错误',
    'security.pending'     => '待处理',
    'security.time'        => '时间',
    'security.project'     => '项目',
    'security.type'        => '类型',
    'security.tag'         => '标签',
    'security.description' => '描述',
    'security.writeback'          => '回写',
    'security.writeback_success'  => '回写成功',
    'security.writeback_failed'   => '回写失败',
    'security.writeback_skipped'  => '未回写',
    'security.writeback_all'      => '全部回写状态',

    // ── 服务监测 ──
    'monitor.page_title'   => '服务监测',
    'monitor.data_overview' => '数据概览',
    'monitor.service_check' => '服务监测',
    'monitor.jenkins'      => 'Jenkins',
    'monitor.git_platforms'=> 'Git 运行时连通性',
    'monitor.git_reachable'=> 'API 可达',
    'monitor.git_unreachable'=> 'API 不可达',
    'monitor.git_no_ref'  => '无运行时引用',
    'monitor.harbor'       => 'Harbor',
    'monitor.system_check' => '系统监测',
    'monitor.build_mode'   => '构建模式',
    'monitor.db_type'      => '数据库类型',
    'monitor.app_version'  => 'APP版本',
    'monitor.env_type'     => '环境类型',
    'monitor.system_time'  => '系统时间',

    // ── 系统/公共枚举 ──
    'system.db_mysql'      => 'MySQL',
    'system.db_sqlite'     => 'SQLite',

    // ── 管理后台通用 ──
    'admin.title'           => '管理后台 — Devops-Glue',
    'admin.login_title'     => '管理登录',
    'admin.login_subtitle'  => 'Devops-Glue 管理后台',
    'admin.account_placeholder' => '账号',
    'admin.password_placeholder' => '密码',
    'admin.home'            => '首页',
    'admin.console'         => '控制台',
    'admin.api_docs'        => 'API 文档',
    'admin.sidebar_monitor' => '监测概览',
    'admin.sidebar_mapping' => '映射管理',
    'admin.sidebar_security'=> '安全审计',
    'admin.sidebar_versions'=> '对接版本',
    'admin.sidebar_mode'    => '构建模式',
    'admin.sidebar_build_records' => '构建记录',
    'admin.sidebar_pull_records'  => '拉取式记录',
    'admin.sidebar_push_records'  => '自定义推送',
    'admin.sidebar_users'   => '用户管理',
    'admin.sidebar_user_list' => '用户列表',
    'admin.sidebar_roles'   => '角色管理',
    'admin.sidebar_password'=> '修改密码',
    'admin.page_lost'       => '管理页面丢失',
    'admin.doc_lost'        => '文档文件丢失',
    'admin.login_page_lost' => '登录页丢失',
    'admin.current_env'     => '当前环境',

    // ── 首页 ──
    'index.title'     => 'Devops-Glue',
    'index.running'   => '运行中',
    'index.subtitle'  => 'Jenkins · Git · Harbor 一站式 DevOps 工具链集成平台',
    'index.admin'     => '登录',
    'index.docs'      => '文档',
    'index.support'   => '技术支持',
    'index.feat_multi_build_title'   => '多链路构建',
    'index.feat_multi_build_desc'    => 'Jenkins · GitLab CI · Gitea CI · Custom_Push 四种 CI 源统一接入，拉取式与推送式正交并存，统一 API 入口，构建触发、状态上报、日志查看完全一致。',
    'index.feat_full_trace_title'   => '全链路映射',
    'index.feat_full_trace_desc'    => 'Pipeline/Job ↔ Git 仓库 ↔ Harbor 镜像，构建→代码→制品完整链路自动关联，Pipeline → Tag 精准追溯。',
    'index.feat_multi_platform_title' => '多平台统一接入',
    'index.feat_multi_platform_desc'  => 'GitLab · GitHub · Gitee · Gitea 统一 API 接口，自建与 SaaS 无差别对接，支持自定义平台扩展。',
    'index.feat_dashboard_title'    => '可视化管理后台',
    'index.feat_dashboard_desc'     => 'Web 管理界面，服务监测、项目拓扑、映射配置、构建日志一目了然。无需改配置文件，表单操作即可完成所有配置，上手即用。',
    'index.feat_api_title'          => '开放 API',
    'index.feat_api_desc'           => '提供完整 RESTful API 与 Swagger 在线文档，支持 API Token 鉴权和细粒度 scope 权限，第三方系统与 CI/CD 流水线无缝对接。',
    'index.feat_health_title'       => '服务健康自检',
    'index.feat_health_desc'        => '实时检测 Jenkins · Git 平台 · Harbor 连通性与版本号，故障快速定位，状态一目了然。',
    'index.footer' => 'Copyright © 2026 jeanslw | Devops-Glue',

    // ── 表单标签 ──
    'form.job_name'         => 'Job 名称',
    'form.git_platform'     => 'Git 平台',
    'form.build_system'     => 'Build 系统',
    'form.status_field'     => '状态',
    'form.git_remote_url'   => 'Git Remote URL',
    'form.project_id'       => 'Project ID',
    'form.web_url'          => 'Web URL',
    'form.project_path'     => '项目路径',
    'form.harbor_repository'=> 'Harbor 仓库',
    'form.please_select'    => '— 请选择 —',
    'form.placeholder_job_name'  => '例: java/registry',
    'form.placeholder_git_remote'=> '例: http://git.company.com/project.git',
    'form.placeholder_project_path' => '例: group/project',
    'form.placeholder_project_id'    => '数字或空',
    'form.placeholder_web_url'       => '项目主页链接',
    'form.placeholder_harbor'    => '例: project/repo',
    'form.placeholder_username'  => 'username',
    'form.placeholder_password'  => '至少 8 位',
    'form.placeholder_current_password' => '输入当前密码',
    'form.placeholder_new_password'     => '至少 8 位',
    'form.placeholder_confirm_password' => '再输入一次',
    'form.commit_sha'            => 'Commit SHA',

    // ── 前端 JS 专用 ──
    'js.login_failed'           => '登录失败',
    'js.scan_in_progress'       => '扫描进行中，请稍候…',
    'js.scanning'               => '正在扫描，请稍候…',
    'js.scan_failed'            => '扫描失败',
    'js.network_error'          => '网络错误',
    'js.save_failed'            => '保存失败',
    'js.no_version_changes'     => '平台版本配置未作更改，无需保存',
    'js.operation_failed'       => '操作失败',
    'js.delete_failed'          => '删除失败',
    'js.activate_failed'        => '启用失败',
    'js.modify_failed'          => '修改失败',
    'js.already_updated'        => '已更新',
    'js.already_added'          => '已新增',
    'js.already_deleted'        => '已删除',
    'js.activated'              => '已启用',
    'js.activate'               => '启用',
    'js.username_required'      => '用户名不能为空',
    'js.password_mismatch'      => '两次密码不一致',
    'js.password_updated'       => '密码已更新，请重新登录',
    'js.delete_confirm'         => '确定删除映射',
    'js.activate_confirm'       => '确认',
    'js.activate_warn_hide'     => '启用后，同仓库的另一条映射将立即隐藏，且下次自动发现时也会被过滤。',
    'js.delete_note'            => '删除后，该映射相关的触发与回写配置将立即失效，且无法恢复。',
    'js.cannot_activate_mode'   => '当前构建模式为 {mode}，无法启用 {item} 项目',
    'js.load_failed'            => '加载失败',
    'js.timeout'                => '超时',
    'js.cannot_connect'         => '无法连接',
    'js.no_configured_platform' => '无已配置平台',
    'js.page_first'             => '首页',
    'js.page_prev'              => '上一页',
    'js.page_next'              => '下一页',
    'js.page_last'              => '末页',
    'js.total_items'            => '共 {total} 条',
    'js.auto_detect'            => '— 自动检测（IP 地址需手动选）—',
    'js.edit_mapping'           => '编辑映射',
    'js.discover_confirm'       => '将自动扫描项目并导入映射表',
    'js.discover_note'          => '将自动扫描当前已配置平台的 Git/Jenkins 项目并导入映射；Git 和 Jenkins 平台，相同仓库地址的项目可能会有重复。',
    // 拓扑图
    'js.topo_manual'            => '手动映射',
    'js.topo_fallback'          => '兜底识别',
    'js.topo_exact'             => '自动识别',
    'js.topo_not_configured'    => '未配置',
    'js.topo_not_linked'        => '未关联',
    'js.topo_unnamed'           => '未命名',
    'js.topo_unnamed_project'   => '未命名项目',
    'js.topo_build_source'      => '构建源',
    'js.topo_git_repo'          => 'Git 仓库',
    'js.topo_harbor_image'      => 'Harbor 镜像',
    'js.topo_total_items'       => '共 {total} 个项目',
    'js.topo_open_harbor'       => '打开 Harbor',
    'js.topo_open_jenkins'      => '打开 Jenkins 项目',
    // 版本管理
    'js.version_source_config'  => '配置文件',
    'js.version_source_admin'   => '管理界面',
    'js.version_source_default' => '系统默认',
    'js.version_config_readonly_title' => '此值由 config/settings.php 显式配置，优先级最高，如需修改请编辑配置文件',
    // Harbor 版本探测 / 机器人账户兼容
    'js.harbor_detected_version' => '探测版本',
    'js.harbor_version_unknown'  => '未知',
    'js.harbor_robot_supported'  => '支持机器人账户（Harbor ≥ 2.2.0，机器人账户使用 secret）',
    'js.harbor_robot_unsupported'=> '不支持机器人账户（Harbor < 2.2.0，机器人账户为 JWT，仅可用于 Docker/Helm CLI）',
    'js.harbor_robot_unknown'    => '无法探测 Harbor 版本，机器人账户支持情况未知',
    'js.harbor_robot_warning'    => '当前配置的是机器人账户，但该 Harbor 版本不支持机器人账户调用 REST API，请改用普通账户（用户名/密码）。',
    // 配置模式
    'js.mode_persisted'         => '持久化',
    'js.mode_temp'              => '临时',
    'js.mode_cannot_detect'     => '无法检测（请先登录）',
    'js.mode_switch_confirm'    => '确定要将构建模式切换为[{mode}]吗？',
    'js.mode_switch_note'        => '此操作会立即生效，会影响后续所有构建任务，并将不再匹配的映射记录改为待启动状态。',
    'js.custom_push_enable_confirm' => '确认启用 Custom_Push？',
    'js.custom_push_toggle_note'  => '启用后，用户 CI 可向 Devops-Glue 推送构建结果回写；关闭后，已激活的 Custom_Push 映射将降为待定状态。',
    'js.custom_push_disable_confirm' => '确认关闭 Custom_Push？',
    'js.stale_tag_cleanup_enable_confirm' => '确认开启过期 tag 清理？',
    'js.stale_tag_cleanup_disable_confirm' => '确认关闭过期 tag 清理？',
    'js.stale_tag_cleanup_note'  => '启用时，将以 Harbor 为准删除系统中已不存在的 tag 记录；关闭时，则停止自动清理，不会删除 Harbor 上的 tag。',
    'js.backfill_tag_enable_confirm' => '确认开启镜像 Tag 日志回填？',
    'js.backfill_tag_disable_confirm' => '确认关闭镜像 Tag 日志回填？',
    'js.backfill_tag_note'  => '启用时，回填 cron 将把日志推导的 tag 经 Harbor 校验存在后补写进产物 tag 记录（仅补缺失项）；关闭时，则停止回写。',
    'js.cp_disabled_hint'       => 'Custom_Push 未启用，选择后将保存为「待定」',
    'js.cp_saved_as_pending'    => 'Custom_Push 未启用，映射已按「待定」保存',
    'js.mode_mode'              => '模式',
    'js.mode_coexist'           => '共存',
    'js.mode_git_repo'          => 'Git 仓库',
    'js.mode_select_all'        => '全选',
    'js.mode_none'              => '无拉取式 CI',
    'js.no_pull_ci'             => '未配置拉取式 CI',
    'js.jenkins_detected_version' => '检测版本',
    'js.jenkins_version_unknown'  => '未知',
    'js.mode_jenkins_name'      => 'Jenkins',
    'js.mode_gitlab_ci_name'    => 'GitLab CI',
    'js.mode_gitea_ci_name'     => 'Gitea Actions',
    'js.mode_custom_push_name'  => 'Custom_Push',
    'js.mode_harbor_name'       => 'Harbor',
    'js.mode_pull_title'        => '拉取式',
    'js.mode_pull_desc'         => 'Glue 触发 CI 构建，CI 拉取代码并推送镜像',
    'js.mode_push_title'        => '推送式',
    'js.mode_push_desc'         => '用户 CI 自行构建并推送镜像，Glue 只接收回写',
    'js.mode_user_ci'           => '用户 CI',

    // ── 语言名称 ──
    'lang.zh_CN' => '中文',
    'lang.en'    => 'English',

    // ── 用户管理 ──
    'user.add_user'    => '＋ 新增用户',
    'user.save_user'   => '保存',
    'user.cancel'      => '取消',
    'user.username'    => '用户名',
    'user.password'    => '密码',
    'user.role'        => '角色',
    'user.systems'     => '可登录系统',
    'user.email'       => '邮箱',
    'user.email_placeholder' => 'user@example.com（选填）',
    'user.invalid_email' => '邮箱格式不正确',
    'user.updated_at'  => '更新时间',
    'user.actions'     => '操作',
    'user.delete'      => '删除',
    'user.created'     => '用户创建成功',
    'user.deleted'     => '用户已删除',
    'user.no_users'    => '暂无用户',
    'user.status'      => '状态',
    'user.enable'      => '启用',
    'user.disable'     => '停用',
    'user.status_updated' => '状态已更新',
    'user.cannot_disable_root' => '内置根账号不允许停用',
    'user.cannot_disable_self' => '不能停用自己的账号',
    'user.confirm_delete' => '确认删除用户',
    'user.edit_user'    => '编辑用户',
    'user.updated'      => '用户已更新',
    'user.password_keep_empty' => '留空则不修改',
    'user.new_password_optional' => '新密码（留空则不修改）',
    'user.cannot_create_admin' => 'CD 侧不允许创建管理员账号',
    'user.cannot_create_super_admin' => '创建超级管理员需要超级管理员权限',
    'user.cannot_delete_admin' => '不允许删除管理员账号',
    'user.cannot_delete_root'  => '内置超级管理员不可删除',
    'user.cannot_delete_self'  => '不能删除自己',
    'user.cannot_edit_admin'   => '不允许编辑管理员账号',
    'user.cannot_edit_root'    => '内置超级管理员不可修改',
    'user.cannot_manage_non_cd' => '服务账号只能管理归属 CD 系统的用户',
    'user.cannot_verify_root'  => '不允许通过该接口校验内置管理员/超级管理员的密码',
    'user.verify_locked'       => '密码校验失败次数过多，请 15 分钟后再试',
    'user.cannot_promote_admin' => '不能将用户提升为管理员',
    'user.cannot_promote_super_admin' => '提升为超级管理员需要超级管理员权限',
    'user.only_one_super_admin' => '系统只允许一个超级管理员',
    'user.nothing_to_update'   => '没有可更新的内容',
    'user.cd_no_ci_access' => 'CD 侧创建的用户不能拥有 CI 登录权限',
    'user.username_exists' => '用户名已存在',
    'user.not_found'    => '用户不存在',
    'user.invalid_username' => '无效的用户名',
    'user.password_required' => '请提供密码',
    'user.invalid_role' => '无效的角色',
    'user.role_not_found' => '角色不存在',
    'user.role_super_admin' => '超级管理员',
    'user.role_deployer' => '部署者',
    'user.role_viewer'   => '只读',
    'user.role_admin'    => '管理员',
    'user.role_ci_admin' => 'CI 管理员',
    'user.role_cd_admin' => 'CD 管理员',
    'user.role_builder'  => '构建者',
    'user.systems_cd'    => '仅 CD',
    'user.systems_ci'    => '仅 CI',
    'user.systems_cd_ci' => 'CD + CI',
    'user.modify_password' => '修改密码',
    'user.modify_password_hint' => '为用户 {user} 设置新密码（至少 8 位）',
    'user.modify_password_confirm' => '确认修改',
    'user.modify_password_mismatch' => '两次输入的密码不一致',
    'user.modify_password_forbidden' => '仅超级管理员可修改他人密码',
    'user.password_updated' => '密码已修改',
    'role.in_use'       => '该角色下还有用户，无法删除',
    'role.no_users'     => '暂无角色',
    'role.add_role'     => '＋ 新增角色',
    'role.edit_role'    => '编辑角色',
    'role.name'         => '角色标识',
    'role.name_required'=> '角色标识不能为空',
    'role.name_placeholder' => '仅小写字母、数字和下划线',
    'role.name_help'    => '仅小写字母、数字、下划线',
    'role.description'  => '显示名称',
    'role.desc_placeholder' => '例：构建者',
    'role.type'         => '类型',
    'role.permissions'  => '权限',
    'role.delete_confirm' => '确定要删除此角色吗？关联用户将失去这些权限。',
    'role.deleted'       => '角色已删除',
    'role.system_role'  => '系统',
    'role.custom_role'  => '自定义',
    'role.sys_cannot_edit'   => '系统内置角色不可编辑',
    'role.sys_cannot_delete' => '系统内置角色不能删除',
    'role.no_roles'     => '暂未创建任何角色',
    'role.no_perms_available' => '暂无可分配权限',
    'role.perm_ci_manage'             => 'CI 配置权限',
    'role.perm_ci_users_manage'       => '用户管理',
    'role.perm_ci_users_list'         => '用户列表',
    'role.perm_ci_users_password'     => '修改密码',
    'role.perm_ci_users_manage_admin' => '角色管理',
    'role.perm_ci_permissions_manage'   => '权限管理',
    'role.perm_ci_permissions_list'     => '权限列表',
    'role.perm_ci_permissions_register' => '权限注册',
    'role.perm_ci_permissions_rules'    => '隐含规则',
    'role.perm_ci_mapping_edit'       => '映射管理',
    'role.perm_ci_platform_edit'      => '对接版本',
    'role.perm_ci_mode_edit'          => '构建模式',
    'role.perm_ci_discover'           => '自动发现',
    'role.perm_ci_trigger'            => '触发构建',
    'role.perm_ci_build-records'      => '构建记录',
    'role.perm_ci_build-records_pull' => '拉取式记录',
    'role.perm_ci_build-records_push' => '自定义推送记录',
    'role.perm_ci_operation-logs'     => '操作日志',
    'role.perm_ci_system'             => '系统信息',
    'role.perm_cd_build-manage'       => '构建管理',
    'role.perm_cd_deploy-manage'      => '部署管理',
    'role.perm_cd_server-manage'      => '服务器管理',
    'role.perm_cd_webshell'           => 'Web Shell',
    'role.perm_cd_deploy-record'      => '部署记录',
    'role.perm_cd_image-registry'     => '镜像仓库',
    'role.perm_cd_resource-monitor'   => '资源监控',
    'role.perm_cd_notification-manage' => '通知管理',
    'role.perm_cd_bot'                => '机器人配置',
    'role.perm_cd_webhook'            => 'WebHook 配置',
    'role.perm_cd_deploy_single'       => 'SSH 部署',
    'role.perm_cd_deploy_docker'       => '部署到 Docker',
    'role.perm_cd_deploy_k8s'          => '部署到 K8S',
    'role.perm_cd_monitor_app'         => '应用资源',
    'role.perm_cd_monitor_system'      => '系统资源',
    'role.perm_cd_monitor_custom'      => '自定义资源',
    'role.perm_cd_monitor_alert'       => '告警规则',
    'role.perm_cd_approval-center'     => '审批中心',
    'role.perm_cd_deploy_approve'      => '审批操作',
    'role.perm_ci_group'              => 'CI 权限',
    'role.perm_cd_group'              => 'CD 权限',
    'role.select_all'                 => '全选',
    'role.deselect_all'               => '取消全选',
    'role.locked'                     => '系统内置',

    // 权限注册/管理 API（数据驱动 RBAC）
    'auth.forbidden'                   => '无权限访问',
    'validation.required'              => '必填字段缺失',
    'validation.invalid_perm_key'      => '权限键格式不合法（应为 xx.yyy 形式）',
    'validation.perm_not_found'        => '权限键不存在于 permissions 表',
    'permission.builtin_protected'     => '系统内置权限不可删除',
    'permission.builtin_immutable'     => '系统内置权限不可修改',

    // 权限管理界面
    'admin.sidebar_permissions'        => '权限管理',
    'admin.sidebar_perm_list'          => '权限列表',
    'admin.sidebar_perm_register'      => '权限注册',
    'admin.sidebar_implied_rules'      => '隐含规则',

    // 权限列表
    'perm.list_title'      => '📋 权限列表',
    'perm.list_hint'       => '系统当前支持的所有权限键。权限由各子系统（CI/CD）通过 API 动态注册，管理员在此查看即可。',
    'perm.key'             => '权限键',
    'perm.description'     => '描述',
    'perm.parent_key'      => '父级键',
    'perm.type'            => '类型',
    'perm.builtin'         => '内置',
    'perm.registered'      => '注册',
    'perm.created_at'      => '注册时间',
    'perm.no_permissions'  => '暂无权限数据',
    'perm.confirm_delete'  => '确定删除此权限？将同时清理其角色与隐含规则关联。',
    'perm.delete_ok'       => '权限已删除',

    // 权限注册
    'perm.register_title'  => '➕ 权限注册',
    'perm.register_hint'   => '手动注册新权限（通常由 CD 项目自动注册，此界面用于特殊情况）。',
    'perm.key_placeholder' => '例: cd.deploy',
    'perm.desc_placeholder'=> '例：部署管理',
    'perm.parent_placeholder'=> '例：cd.deploy-manage（可选）',
    'perm.key_format'      => '格式：小写字母、数字、点号、下划线',
    'perm.register_btn'    => '注册',
    'perm.key_required'    => '权限键不能为空',
    'perm.desc_required'   => '描述不能为空',
    'perm.register_ok'     => '权限注册成功',

    // 隐含规则
    'implied.title'        => '🔗 隐含规则管理',
    'implied.add_rule'     => '＋ 新增规则',
    'implied.list_hint'    => '权限隐含关系：勾选源权限时自动拥有目标权限。',
    'implied.source_key'   => '源权限',
    'implied.target_key'   => '目标权限',
    'implied.source_hint'  => '拥有此权限时',
    'implied.target_hint'  => '自动拥有此权限',
    'implied.no_rules'     => '暂无隐含规则',
    'implied.both_required'=> '源权限和目标权限都必须选择',
    'implied.cannot_same'  => '源权限和目标权限不能相同',
    'implied.confirm_delete'=> '确定删除此隐含规则？',
    'implied.add_ok'       => '隐含规则添加成功',
    'implied.delete_ok'    => '隐含规则已删除',
    'implied.builtin_protected' => '系统内置隐含规则不可删除',

    // API Token 管理
    'api_token.menu'            => 'API 管理',
    'api_token.add'             => '＋ 新增 Token',
    'api_token.name'            => '名称',
    'api_token.name_placeholder'=> '例：CD 服务账号 / Jenkins 扫描回写',
    'api_token.scopes'          => '接口权限（Scope）',
    'api_token.expires'         => '过期日期',
    'api_token.expires_hint'    => '留空表示永不过期',
    'api_token.expires_invalid' => '日期格式应为 yyyy/mm/dd，如 2026/08/13',
    'api_token.note'            => '备注',
    'api_token.note_placeholder'=> '用途说明（可选）',
    'api_token.created_title'   => '✅ Token 创建成功',
    'api_token.created_hint'    => '请立即复制保存，此明文 Token 仅显示这一次。',
    'api_token.copy'            => '复制',
    'api_token.status'          => '状态',
    'api_token.created_at'      => '创建时间',
    'api_token.empty'           => '暂无 Token',
    'api_token.never'           => '永不过期',
    'api_token.active'          => '有效',
    'api_token.expired'         => '已过期',
    'api_token.revoke'          => '撤销',
    'api_token.name_required'   => '请填写名称',
    'api_token.copied'          => '已复制',
    'api_token.copy_failed'     => '复制失败',
    'api_token.confirm_revoke'  => '确认撤销该 Token？撤销后立即失效。',
    'api_token.revoked'         => '已撤销',
    'api_token.delete'          => '删除',
    'api_token.confirm_delete'  => '确认删除该 Token？删除后记录不可恢复。',
    'api_token.deleted'         => '已删除',
    'api_token.disabled'        => '已禁用',
    'api_token.super_admin_only'=> '仅超级管理员可访问',
    'api_token.scope_forbidden' => 'API Token 无权访问此接口',
    'api_token.service_unavailable' => 'API Token 服务不可用',

    // API Token Scope 目录
    'api.scope.main'            => 'MAIN（只读）',
    'api.scope.git'             => 'GIT（只读）',
    'api.scope.harbor_read'     => 'Harbor 查询',
    'api.scope.harbor_scan'     => 'Harbor 扫描触发',
    'api.scope.build_read'      => '构建查询',
    'api.scope.build_write'     => '构建执行（触发/重试/取消）',
    'api.scope.build_report'    => '构建回写（scan-sync/commit-status/report）',
    'api.scope.dashboard'       => '监控看板（只读）',
    'api.scope.rbac_user_write' => 'RBAC 用户写（建号/改号/删号）',
    'dashboard.query_failed'    => '看板查询失败',

    // ── OAuth2 Provider ──
    'oauth.login_title'      => '登录 Devops-Glue',
    'oauth.invalid_request'  => '无效的 OAuth 请求（client_id 或 redirect_uri 不匹配）',

    // ── 操作日志 ──
    'oplog.menu'               => '操作日志',
    'oplog.action'             => '操作',
    'oplog.target'             => '对象',
    'oplog.detail'             => '详情',
    'oplog.ip'                 => 'IP',
    'oplog.result'             => '结果',
    'oplog.time'               => '时间',
    'oplog.success'            => '成功',
    'oplog.failure'            => '失败',
    'oplog.operator_type'      => '操作人类型',
    'oplog.admin'              => '管理员',
    'oplog.api_token'          => 'API Token',
    'oplog.result_all'         => '全部',
    'oplog.date_from'          => '起始日期',
    'oplog.date_to'            => '结束日期',
    'oplog.search'             => '查询',
    'oplog.reset'              => '重置',
    'oplog.empty'              => '暂无操作记录',
    'oplog.username_placeholder' => '模糊搜索用户名',
    'oplog.action_placeholder'   => '如 login / create_user',
    'oplog.act_login'          => '登录',
    'oplog.act_login_failed'   => '登录失败',
    'oplog.act_login_locked'   => '登录锁定',
    'oplog.act_logout'         => '退出登录',
    'oplog.act_change_password' => '修改密码',
    'oplog.act_auto_discover'  => '自动发现',
    'oplog.act_build_trigger'  => '触发构建',
    'oplog.act_build_retry'    => '重试构建',
    'oplog.act_build_cancel'   => '取消构建',
    'oplog.act_create_mapping' => '新增映射',
    'oplog.act_update_mapping' => '更新映射',
    'oplog.act_delete_mapping' => '删除映射',
    'oplog.act_update_platform_version' => '更新平台版本',
    'oplog.act_update_build_mode' => '更新构建模式',
    'oplog.act_create_user'    => '新增用户',
    'oplog.act_update_user'    => '更新用户',
    'oplog.act_reset_user_password' => '重置用户密码',
    'oplog.act_set_user_status' => '启用/停用用户',
    'oplog.act_delete_user'    => '删除用户',
    'oplog.act_create_role'    => '新增角色',
    'oplog.act_update_role'    => '更新角色',
    'oplog.act_delete_role'    => '删除角色',
    'oplog.act_register_permission' => '注册权限',
    'oplog.act_delete_permission' => '删除权限',
    'oplog.act_create_implied_rule' => '新增隐含规则',
    'oplog.act_delete_implied_rule' => '删除隐含规则',
    'oplog.act_create_api_token' => '创建 API Token',
    'oplog.act_revoke_api_token' => '撤销 API Token',
    'oplog.act_delete_api_token' => '删除 API Token',
    'oplog.act_migrate_schema' => '迁移数据库',
    'oplog.act_backup_database' => '备份数据库',

    // 系统信息面板
    'sys.menu'            => '系统设置',
    'sys.tab_platform'    => '🔌 平台管理',
    'sys.tab_system_info' => '🗄️ 数据管理',
    'sys.panel_system_info' => '🖥️ 系统信息',
    'sys.panel_database'  => '🗄️ 数据库',
    'sys.driver'          => '数据库驱动',
    'sys.app_version'     => '代码版本',
    'sys.schema_version'  => 'Schema 版本',
    'sys.is_current'      => 'Schema 状态',
    'sys.php_version'     => 'PHP 版本',
    'sys.current'         => '✅ 已是最新',
    'sys.outdated'        => '⚠️ 与代码版本不一致（需迁移）',
    'sys.none'            => '（未记录）',
    'sys.tables'          => '核心数据表',
    'sys.tables_toggle'   => '📋 数据表',
    'sys.platforms'       => '平台接入状态',
    'sys.configured'      => '已配置',
    'sys.not_configured'  => '未配置',
    'sys.cred_set'        => '凭证已配置',
    'sys.cred_unset'      => '凭证未配置',
    'sys.migrate_btn'     => '🛠 迁移数据库',
    'sys.migrate_confirm' => '确定要执行数据库迁移吗？',
    'sys.migrate_note'    => '将创建缺失的数据表并应用种子数据（幂等，安全）。',
    'sys.migrated'        => '迁移完成',
    'sys.migrate_failed'  => '迁移失败',
    'sys.backup_btn'      => '💾 备份数据库',
    'sys.backup_list'     => '备份文件',
    'sys.backup_confirm'  => '确定要执行数据库备份吗？将生成 zip 备份文件并保存到 backups 目录（仅备份，不提供恢复）。',
    'sys.backup_done'     => '备份完成',
    'sys.backup_failed'   => '备份失败',
    'sys.backup_no_tables' => '没有任何应用表可备份（数据库未初始化？）',
];

