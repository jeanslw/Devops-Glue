    ...
    try {
        ...
        tbody.innerHTML = pageRecords.map((r, j) => {
            ...
            return '<tr>'
                ...
                + '<td id="pull-tag-' + i + '">' + (r.tag ? '<code style="font-size:11px;">' + esc(r.tag) + '</code>' : (r.can_resolve_tag ? '<button class="btn btn-sm" onclick="resolvePullTag(' + i + ')">🔍 ' + esc(__.t('pull.resolve_tag')) + '</button>' : '—')) + '</td>'
                + '<td>' + (time ? esc(time) : '—') + '</td>'
                + '<td>' + actionsCell + '</td>'
                + '</tr>';
        }).join('');
...
export async function resolvePullTag(idx) {
    ...
    try {
        ...
        cell.innerHTML = tag ? '<code style="font-size:11px;">' + esc(tag) + '</code>' : '—';
    } catch (e) {
        cell.innerHTML = '<button class="btn btn-sm" onclick="resolvePullTag(' + idx + ')">🔍 ' + esc(__.t('pull.resolve_tag')) + '</button>';
